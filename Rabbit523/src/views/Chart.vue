  ...
  methods: {
    ...
    updateData(event) {
      ...
      if (data.channel == "ticker" && this.lastTimestamp > 0) {
        if (
          this.lastTimestamp + this.timeframe * 1000 >
          data.data.time * 1000
        ) {
          var lastCandleData = this.candleData.pop();
          if (lastCandleData[2] < data.data.last)
            lastCandleData[2] = data.data.last;
          if (lastCandleData[3] > data.data.last)
            lastCandleData[3] = data.data.last;
          lastCandleData[4] = data.data.last;
          this.candleData.push(lastCandleData);
        } else {
          this.lastTimestamp = parseInt(
            this.lastTimestamp + this.timeframe * 1000
          );
    ...
    onopen: function (event) {
      ...
      axios
        .get(
          "/api/markets/BTC/USD/candles?resolution=" +
            this.timeframe +
            "&start_time=" +
            timestamp,
          {}
